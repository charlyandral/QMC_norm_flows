...
#%%
n_size = 2**16
dims = np.repeat(np.arange(2,11),5)
n_rep = 50
...
def loop(i):
    n_dim = dims[i]
    model = MaskedCouplingRQSpline(n_dim, 6, [32,32], 8, jax.random.PRNGKey(i))
    n_loop_training = 5
    @jax.jit
    def target_dualmoon(x, data=None):
        """
    ...
    sampler.trainning_flow()
    common_dict = {"n_sample": n_size, "n_rep": n_rep, "seed": 10000 * (i+1), "n_jobs": 1}
    simulations = [
    ...
    return df_melt
out  = Parallel(n_jobs=30, backend="threading", verbose=10)(
            delayed(loop)(i) for i in range(len(dims))
        )
df_melt = pd.concat(out)
df_melt.to_pickle("all.zip")